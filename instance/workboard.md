...
| V3-1.5 | `meteorite/run.sh` — the clean-rebuild proof itself, absent from v3 while the plan measured v3 against it. | `bash meteorite/run.sh` from a clean clone in Docker exits 0 | **implemented; current verdict NO-GO, awaiting independent review/landing** — clean `ubuntu:24.04`, remote clone at a pinned SHA, stage-1 bootstrap, full `bun test`, rendered-unit drift check, fail-closed report, and named unproven activation/watchdog/Telegram boundaries. The first real run reached `bun test` and failed closed: the clean clone lacks the local donor ref/test prerequisites and the trusted `/usr/local/bin/bun` boundary required by existing tests. |
