...
- <!-- status: open --> **W-21 — the production database contained tables from a migration that is not in git** (found 2026-08-01 during the first real deploy). `schema_migrations` recorded `002_gmail_import_storage.sql`, a file that exists nowhere in the repository, and the live database carried `gmail_imported_messages` (8 rows, with an `import_run_id` column no repo code references) plus `gmail_import_runs` (referenced by NO repo file and created by NO repo migration). Those orphans collided with `packages/gmail/migrations/0002_gmail_document_import.sql`, which had never been applied, so EVERY restart failed at startup — the stand only appeared healthy because it had not restarted since 18:23. Preserved by renaming to `*_orphan_20260801` rather than dropping; the phantom `schema_migrations` row was removed and gmail/0002 then applied cleanly. Root cause not yet established: most likely a lane ran migrations against the shared production DATABASE_URL instead of a disposable database. That is the real defect to fix — a lane must not be able to migrate his live accounting database. Also a Hard Floor 5 breach in the data layer: the database could not be reproduced from git.
...
- <!-- status: open --> **PR-12 — authentication is NOT PORTED and has been REJECTED three times** (2026-08-01). The app is publicly reachable at agentic.bpa.pro holding live QuickBooks/Gmail/Drive tokens and a real company's books, with no login at all. Round 1 rejected: any self-registered, unverified user reached shared integrations and accounting routes. Round 2 self-reported NO-GO: entrance-only scoping and a two-org isolation lock that used a SYNTHETIC database response. Round 3 rejected: forced RLS missed the Gmail document and reconciliation tables, and the runtime DB role was never checked for SUPERUSER/BYPASSRLS — either silently disables every policy. Enforcement stays DEFAULT OFF; turning it on is the operator's decision, never a lane's.
...
- <!-- status: open --> **W-22 — landed work did not reach the live stand for six hours** (found 2026-08-01). Everything merged between 18:23 and 00:10 sat undeployed while the stand served a stale build; the operator would have woken, clicked, and seen nothing new. There was no deployment path in git and no staleness alarm. A deploy mechanism now exists (`deploy/live-stand.sh`, release directories + `current` symlink) but its review REJECTED it for restarting the service before detecting migration conflicts and for not proving rollback. Two real deploys failed on it tonight and rolled back; a third defect it does not handle is directory permissions — a fresh release built as root is mode 0700 and the service user cannot enter it.
- <!-- status: open --> **W-23 — the product read the infrastructure repo at RUNTIME** (found 2026-08-01). `scripts/operations.mjs` loaded the canonical secret pattern from `/root/bpa-dev-infrastructure/gate/land-lib.sh` at startup; the service user cannot read `/root`, so the build crash-looped on EACCES and two deploys rolled back. Root cause is an orchestrator instruction that told lanes to secret-scan "using the canonical pattern from gate/land-lib.sh" — meant for the lane's own build-time scan, taken as a runtime dependency. Pattern is now vendored at `deploy/canonical-secret-pattern.sh`. The general rule this violates is the operator's own: «продукт і його розробка — це різні речі, які треба розділяти». A check should make a cross-repo runtime read impossible, not merely fixed once.
