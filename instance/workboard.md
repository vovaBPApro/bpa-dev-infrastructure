...
| V3-1.5 | `meteorite/run.sh` — the clean-rebuild proof itself, absent from v3 while the plan measured v3 against it. **Re-scoped 2026-08-03:** the donor is a *product* meteorite (clones `PRODUCT_GIT_URL`) and no product repo exists; the infrastructure meteorite was never a script, only a hand-run procedure that FAILED on 2026-07-31 (`v2-deprecated:reports/meteorite-test.md`). | `bash meteorite/run.sh` from a clean clone in Docker exits 0 | **PARKED 2026-08-03** — three HR-1726 rounds, no ACCEPT. Happy path proven by execution (credential-free HTTPS, temporary tracked-remote refs, no host state, truthful source label, container and refs removed). Blockers: published refs leak on signal/crash with no verdict emitted, and a failed cleanup cannot revise a report that already says `result: clean`. Retained on branch `ag-s3-1-r3` (`a7e3b761fad157d196a000633df313a7f7bba696`, pushed to origin, do not reap). Evidence and the lock the recut must satisfy: `instance/parked/V3-1.5-meteorite-2026-08-03.md`. **Its one real run says v3 does NOT currently survive a clean rebuild** — 17 failures, one cause fixed in `787bb40`, the `/usr/local/bin/bun` trust boundary still open. |
