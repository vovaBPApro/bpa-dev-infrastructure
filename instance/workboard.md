...
- <!-- status: open --> **W-29 — bidirectional Telegram history logging (ML-11) is landed but not live** (found 2026-08-01, operator asked to re-read the full Telegram log for problems). `daemon/history-logger.ts` writes to `TELEGRAM_STATE_DIR/history/messages-YYYY-MM.jsonl` (`/root/.claude/channels/telegram/history/` per the deployed unit's `TELEGRAM_STATE_DIR`), but that directory does not exist on the live host — only `.env`, `access.json`, `daemon.pid`, `daemon/`, and `inbox/` are present. The daemon unit is active, so either the logger call site is not wired into the live message path, or it is silently failing closed. Same class as W-17 (deployed vs. tracked drift invisible without a diff) and the ML-GOV pattern (a landed capability that never actually took effect). No verbatim replay of Telegram history is possible until this is fixed — the record does not exist, not merely redacted. Needs a lane: confirm wiring in `daemon/server.ts`, reproduce locally, fix or explain, then re-verify write on the live host.
