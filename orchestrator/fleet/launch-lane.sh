...
if [[ "$EUID" -ne "$service_uid" ]]; then
  ...
  exec setpriv --reuid="$service_uid" --regid="$service_gid" --init-groups \
    env -u SUDO_UID -u SUDO_GID -u SUDO_USER HOME="$LANE_SERVICE_HOME" USER="$LANE_SERVICE_USER" LOGNAME="$LANE_SERVICE_USER" \
    XDG_RUNTIME_DIR="/run/user/$service_uid" LANE_SERVICE_CONFIG="$service_config" BUN_BIN="$BUN_BIN" \
...
# Prove the exact user manager's enforcement before deriving or creating any
# lane artifact path. An unavailable, failed, or timed-out proof is refusal.
network_probe="${LANE_NETWORK_PROBE:-$SCRIPT_DIR/network-boundary-probe.sh}"
[[ -x "$network_probe" ]] || die "lane network capability probe is unavailable: $network_probe"
BUN_BIN="$BUN_BIN" "$network_probe" || die 'lane network capability probe failed'
