...
- Keep raw inbound Human messages only in ignored `instance/decisions/inbox.jsonl`. Track `instance/decisions/triage.jsonl` as durable governance, but only with `msg_id`, a `chatter`/`directive` verdict, category-only `category` and `reason` tokens, and `triaged_by`/`triaged_at` provenance. Quotes, message text, and other free-text fields are forbidden and the ledger checker must fail closed on them.
