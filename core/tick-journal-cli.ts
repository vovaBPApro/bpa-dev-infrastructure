...
try {
  if (command === "record") {
    ...
    store.db.query(`INSERT INTO tick_producer_state (producer_id, interval_number, boot_id, updated_at) VALUES (?, ?, ?, ?)
      ON CONFLICT(producer_id) DO UPDATE SET interval_number=excluded.interval_number, boot_id=excluded.boot_id, updated_at=excluded.updated_at`).run(producerId, current, bootId, observedAt);
    ...
    const result = store.accountMissedTicks(intervals);
