...
(
  trap '' TERM
  sleep "$SURVIVOR_READY_DELAY_SECONDS"
  exec 9>"$SURVIVOR_LOCK_FILE"
  flock -x 9
  printf 'lock-held\n' >"$SURVIVOR_READY_FIFO"
  while :; do sleep 1; done
) &
IFS= read -r ready <"$SURVIVOR_READY_FIFO"
[[ "$ready" == lock-held ]] || exit 98
touch "$SURVIVOR_READY_OBSERVED"
...
trusted_prover_sha="$(git -C "$fixture/repo" rev-parse HEAD)"
# The FIFO is a causal handshake: the prover cannot enter wait until the child
# actually holds the lock, and the test timeout remains the one bound on a
# broken handshake. The injected 200ms scheduling delay is deliberately longer
# than r3's ten 10ms polls, so restoring that polling loop makes this fixture
# deterministically report rebuild-proof-failed instead of the required timeout.
mkfifo "$fixture/survivor.ready.fifo"
if SURVIVOR_LOCK_FILE="$fixture/survivor.lock" SURVIVOR_READY_FIFO="$fixture/survivor.ready.fifo" \
    SURVIVOR_READY_OBSERVED="$fixture/survivor.ready.observed" SURVIVOR_READY_DELAY_SECONDS=0.2 \
    FAKE_DOCKER_STATE="$fixture/container.state" \
...
if ! grep -Fq 'LAND meteorite blocker=rebuild-proof-timeout' "$fixture/timeout.out"; then
  echo 'timeout rehearsal returned the wrong blocker:' >&2
  sed 's/^/  /' "$fixture/timeout.out" >&2
  exit 1
fi
...
test -e "$fixture/survivor.ready.observed" || { echo 'survivor rehearsal never held its lock' >&2; exit 1; }
