...
- Keep raw inbound Human messages only in ignored `instance/decisions/inbox.jsonl`. Track `instance/decisions/triage.jsonl` as durable governance with `msg_id`, a `chatter`/`directive` verdict, category-only `category` and `reason` tokens, a verbatim `quote`, and `triaged_by`/`triaged_at` provenance. The ledger checker must accept the operator's verbatim words and fail closed when the quote matches the canonical secret signature from `gate/land-lib.sh`; that pattern has one home and must not be copied.
