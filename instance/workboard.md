...
- <!-- status: open --> **W-37 — daemon injects terminal-alert banners into the very pane the watcher watches** (found 2026-08-01 during the W-33 fifth ignition). Terminal alerts are delivered as channel messages into the orchestrator session, so every alert is re-rendered in the watched pane — an inherently circular topology; classifier hardening (W-33 round 9) breaks the current loop, but the architecture invites the next variant. Candidate fix (daemon/server.ts, Tier A, needs its own lane + review): deliver terminal-alert-source messages to the session WITHOUT the banner vocabulary in the rendered form, or route them to the daemon log/watchdog state only and let the session poll. Not scoped into round 9 deliberately. Progress 2026-08-01: coder lane `w37-alert-routing` delivered candidate `1409cf6` (+evidence `013c3bf`, branch `ag-w37-alert-routing`): alerts are journaled in FULL; the connected MCP session (or the tmux fallback) receives only an inert pointer line (`terminal-alert: kind=<encoded-kind> nonce=<nonce> — details in daemon journal`); when neither route works delivery still throws `orchestrator unavailable`. Red-before evidence: restoring the former tmux-first/full-frame behavior fails the MCP-preference and inert-paste locks. In-lane: 55/55 targeted tests + tsc 0; full daemon suite was an in-lane NO-GO on loopback-dependent timeouts — host re-run at the SHA 12:33: 248 pass / 0 fail across 23 files (64s), so the in-lane NO-GO was environmental. Independent Tier A review dispatched 12:36 on lane `review-w37-alert-routing` (Codex, branch `ag-review-w37-alert-routing`), briefed that pointer-line classifier-inertness under every rendering/truncation is the blocking property, plus no-silent-drop (journal + throw), W-33 rounds 3-10 boundary intact at the SHA, and a certified watcher re-arm precondition on ACCEPT. Watcher stays OFF until this lands and re-arm is a deliberate announced step. SIXTH ignition 2026-08-01 12:34:20–12:34:23: the 12:26 watchdog respawn re-armed the on-disk (round-10) watcher — `orchestrator/launch.sh` arms it UNCONDITIONALLY and fail-closed (arming failure kills the launch, launch.sh:375), so the deliberate-OFF decision cannot survive any respawn without a code change. Round-10 emitted encoding HELD (headers arrived middle-dot-encoded, no header re-classification); the loop ran through the DECLARED residual exactly as predicted: the orchestrator's own workboard edit rendered genuine failure vocabulary in the watched pane, each alert quoted that text verbatim in its payload, the injected alert re-rendered in the pane, and payloads re-classified at ~2 alerts/s (8 alerts; final one visibly quotes a previous alert's payload). Alerts stayed internal-audience; operator chat untouched. Mitigated 12:35: pipe-pane detached + watcher killed (pipe=0, no watcher process; live session survived; both running lanes unaffected). Decision: no separate knob lane under current quota pressure — W-37 IS the fix for the return path; until it lands every respawn may re-ignite and the standing mitigation is detach+kill. Deploy sequencing decided: land W-37 → land W-31 round 4 → one announced deliberate relaunch through the scoped launcher → daemon restart deploys the W-37 delivery path; only then is watcher re-arm discussable.
