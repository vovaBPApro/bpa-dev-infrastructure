...
async function fixture(runnerExit: number, failDeletion = false) {
...
case "$*" in
  ...
  *"push origin :"*) [[ "$FAIL_DELETION" == 1 ]] && exit 1 || exit 0 ;;
  *"push origin"*) exit 0 ;;
  *) exit 90 ;;
esac
`);
  await writeFile(join(bin, "bash"), `#!/bin/bash
	...
	printf '%s\\n' '# report' '- result: clean' '- blocker: none' > "$METEORITE_REPORT"
	exit "$RUNNER_EXIT"
`);
  await chmod(join(bin, "git"), 0o755);
  await chmod(join(bin, "bash"), 0o755);
  return { sha, trace, report, env: { ...process.env, PATH: `${bin}:${process.env.PATH}`, TRACE: trace, SHA: sha, RUNNER_EXIT: String(runnerExit), FAIL_DELETION: failDeletion ? "1" : "0", METEORITE_REPORT: report, METEORITE_CREATED_AT: "1700000000" } };
}
...
describe("meteorite candidate publication", () => {
  test("publishes the SHA, names the mechanism, and cleans up after success", async () => {
    ...
    expect(trace).toMatch(new RegExp(`push origin ${f.sha}:refs/meteorite-candidates/1700000000-[0-9]+-${f.sha}/candidate`));
    expect(trace).toContain("mechanism=temporary tracked-remote refs refs/meteorite-candidates/");
    expect(trace).toMatch(/push origin :refs\/meteorite-candidates\/.*\/candidate/);
    expect(trace).toMatch(/push origin :refs\/meteorite-candidates\/.*\/v2-deprecated/);
  });
  ...
  test("revises the durable report to NO-GO when deletion fails", async () => {
    const f = await fixture(0, true);
    const run = Bun.spawnSync(["/bin/bash", publisher, "--ref", f.sha], { env: f.env });
    expect(run.exitCode).not.toBe(0);
    const report = await readFile(f.report, "utf8");
    expect(report).toContain("- result: NO-GO");
    expect(report).toContain("- blocker: published meteorite ref cleanup failed: refs/meteorite-candidates/");
