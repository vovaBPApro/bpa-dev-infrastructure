...
- A terminal report may use the optional structured `review:` field only to claim a completed review. The field name is case-insensitive, may have leading whitespace, and permits whitespace before the colon; field-looking lines inside backtick or tilde fenced code blocks are examples, not fields. When the field exists outside a fence, `completion-guard.ts` requires the sibling artifact `<branch>.review.md` to exist, contain exactly one `verdict: ACCEPT`, and name the report commit in exactly one `reviewed-sha:` field. Outstanding review belongs in `remaining:` and must not be represented by `review:`. Other useful extra fields remain allowed.
