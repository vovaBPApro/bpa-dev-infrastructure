...
| V3-0.12 | **New.** Close the two measured shell-tier exclusions without weakening their subject assertions. | both excluded shell tests pass; the pinned inventory remains nine; `excludedShellTests` is empty; subject mutations prove both locks red | **NO-GO** — the rollback privilege fixture now passes beneath a traversable system temporary root. Updating the watchdog fixture to the current mission/manager/lane create contract exposed a subject-boundary incompatibility: `mission-cli status` emits snake_case mission rows, omits lane `updatedAt`, and has no `leases` collection, while `watchdog.sh` requires camelCase fields and `status.leases`; the nudge is therefore not emitted. Fixing that production contract is outside this fixture-only row. |
