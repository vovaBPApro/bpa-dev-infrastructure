...
- A terminal report may use the optional structured `review:` field only to claim a completed review. When that field exists, `completion-guard.ts` requires the sibling artifact `<branch>.review.md` to exist, contain exactly one `verdict: ACCEPT`, and name the report commit in exactly one `reviewed-sha:` field. Outstanding review belongs in `remaining:` and must not be represented by `review:`. Other useful extra fields remain allowed.
